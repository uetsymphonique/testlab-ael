...
The developer previously saved MSSQL credentials for `IIS01` in Windows Credential Manager via SSMS. Rather than the PowerShell `CredentialManager` module - its NuGet bootstrap is unavailable in the lab and `Get-StoredCredential` is a heavily signatured telemetry surface - the adversary pushes a small custom Go utility (`credvault.exe`) through the TONESHELL file-transfer channel. Because the implant already runs in `labuser`'s session, `credvault.exe` calls `CredEnumerateW` and `CredReadW` via advapi32 in one pass (`dump Microsoft:SSMS`) to recover the plaintext `svc_app_dev` password in-session - no offline cracking, no PowerShell or `cmdkey.exe` anywhere in the chain.
