...
3. Confirm the collected files are present on WS01:
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports |
| - | - | - | - | - | - | - | - | - | - | - | - | - |
| TONESHELL FILE_DOWNLOAD transfers go-thehash.exe to C:\Windows\Temp\ on WS01 | Command and Control | T1105 | Ingress Tool Transfer | Windows | `EssosUpdate.exe` (TONESHELL implant) on WS01 writes a new PE file `go-thehash.exe` to `C:\Windows\Temp\` (Sysmon EC=11, creator `EssosUpdate.exe`) - a signed Microsoft binary acting as a C2 implant depositing an inbound non-Microsoft executable into the system temp directory, with no baseline for file-write activity from this process | Calibrated - Not Benign | - | TONESHELL implant receives FILE_DOWNLOAD (id=3) task and writes `go-thehash.exe` to `C:\Windows\Temp\` on WS01 | WS01 (10.12.10.30) | TESTLAB\labuser | [controlShell](../resources/payloads/rce-and-c2/mustang-panda-emulation/controlShell/) | - |
...
2. ☣️ Deploy the xpagent in-database C2 agent on IIS01:
   ...
   Copies `xpagent_init.sql` from `controlServer/sql/` to the C2 payloads directory, pushes it to `C:\Windows\Temp\` on WS01 via TONESHELL FILE_DOWNLOAD, then runs `sqlcmd -i` against `IIS01\SQLEXPRESS`. The DDL creates the `xpagent` database with `dbo.cmd` / `dbo.out` tables, an AFTER INSERT trigger, a Service Broker queue + service, and the `agent_worker` activation procedure. Subsequent steps use `xpexec` to dispatch commands through this channel instead of the sp_OA file-staging path.

   - ***Expected Output***
     ```text
     [*] xpagent_init: pushing SQL to WS01 ...
     [*] xpagent_init: running xpagent_init.sql on IIS01 ...
     [+] xpagent_init done
     ```
...
| TONESHELL EXEC del deletes xpagent_init.sql from C:\Windows\Temp\ on WS01 | Stealth | T1070.004 | Indicator Removal: File Deletion | Windows | `EssosUpdate.exe` spawns `cmd.exe /c del /f C:\Windows\Temp\xpagent_init.sql` on WS01 - TONESHELL implant removing the DDL staging file post-deployment; same process chain as the xpstage-hex INSERT SQL deletion row in Step 4 | Not Calibrated - Not Benign | staging | WS01 implant runs EXEC task `cmd /c del /f C:\Windows\Temp\xpagent_init.sql` removing the DDL staging script after deployment | WS01 (10.12.10.30) | TESTLAB\labuser | - | - |
...
| xpagent sqlcmd INSERT dispatches service check commands; xprun dispatches EfsPotato via sp_OA on IIS01 | Lateral Movement | T1021 | Remote Services | Windows | `sqlcmd.exe` spawned by `EssosUpdate.exe` on WS01 makes repeated short-lived TCP/1433 connections to `10.12.10.20` - one `sqlcmd` invocation per `xpexec` call dispatching service check and escalation commands to IIS01; same network anomaly as the initial MSSQL authentication row, observable through a sustained pattern of short-lived sqlcmd process spawns from the TONESHELL implant | Not Calibrated - Not Benign | redundant@T1021 | WS01 implant runs `sqlcmd -Q` to: **(a)** INSERT `sc query EFS` and (conditional: `sc start EFS`) into `xpagent.dbo.cmd` via xpexec; **(b)** dispatch CertEnrollSvc.exe via `xprun` (sp_OA `WScript.Shell.Run` — no xpagent, no cmd.exe from SQL Server); file verification (`xpfile exists`), output readback (`xpfile cat`), and cleanup (`xpfile del`) use direct `sqlcmd -Q` T-SQL | WS01 (10.12.10.30) | TESTLAB\labuser | [controlShell](../resources/payloads/rce-and-c2/mustang-panda-emulation/controlShell/) | - |
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports |
| - | - | - | - | - | - | - | - | - | - | - | - | - |
| xpagent sqlcmd INSERT dispatches net.exe discovery commands to xpagent.dbo.cmd queue | Lateral Movement | T1021 | Remote Services | Windows | `sqlcmd.exe` spawned by `EssosUpdate.exe` on WS01 makes repeated TCP/1433 connections to `10.12.10.20` - four INSERT dispatches submitting `net` discovery commands to the xpagent queue on IIS01; same network anomaly as prior T1021 INSERT dispatch rows (Steps 3 and 4) | Not Calibrated - Not Benign | redundant@T1021 | WS01 implant runs `sqlcmd -Q "INSERT INTO xpagent.dbo.cmd(cmd) VALUES(...)"` four times dispatching `net group /domain`, `net group "Domain Admins" /domain`, `net user /domain`, and `net user administrator /domain` to the xpagent Service Broker queue on IIS01 | WS01 (10.12.10.30) | TESTLAB\labuser | [controlShell](../resources/payloads/rce-and-c2/mustang-panda-emulation/controlShell/) | - |
...
| agent_worker activation proc invokes xp_cmdshell running net group "Domain Admins" /domain on IIS01 | Discovery | T1069.002 | Permission Groups Discovery: Domain Groups | Windows | `net.exe group "Domain Admins" /domain` executed as `NT SERVICE\MSSQL$SQLEXPRESS` on IIS01 via process chain `sqlservr.exe → cmd.exe → net.exe` - Domain Admins membership query via SAM-R issued by the MSSQL service account; error outcome (`System error 5`) does not prevent the process creation event from surfacing | Not Calibrated - Not Benign | redundant@T1069.002 | IIS01 `agent_worker` calls `xp_cmdshell 'net group "Domain Admins" /domain'`; `net.exe` queries Domain Admins membership as `IIS01$`; `System error 5` is an acceptable outcome - process creation observable regardless | IIS01 (10.12.10.20) | NT SERVICE\MSSQL$SQLEXPRESS | - | - |
...
| agent_worker activation proc invokes xp_cmdshell running net user administrator /domain on IIS01 | Discovery | T1087.002 | Account Discovery: Domain Account | Windows | `net.exe user administrator /domain` executed as `NT SERVICE\MSSQL$SQLEXPRESS` on IIS01 via process chain `sqlservr.exe → cmd.exe → net.exe` - targeted domain administrator account detail query via SAM-R; same process creation signal as the `net user /domain` row under the same technique | Not Calibrated - Not Benign | redundant@T1087.002 | IIS01 `agent_worker` calls `xp_cmdshell 'net user administrator /domain'`; `net.exe` queries the full domain administrator account record via SAM-R to DC as `IIS01$` | IIS01 (10.12.10.20) | NT SERVICE\MSSQL$SQLEXPRESS | - | - |
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports |
| - | - | - | - | - | - | - | - | - | - | - | - | - |
| TONESHELL FILE_DOWNLOAD transfers hex INSERT SQL staging file to C:\Windows\Temp\ on WS01 | Command and Control | T1105 | Ingress Tool Transfer | Windows | `EssosUpdate.exe` (TONESHELL implant) on WS01 writes a hex-encoded INSERT SQL file to `C:\Windows\Temp\` - signed Microsoft binary with no baseline for depositing SQL staging files to the system temp directory; same file-write signal as the xpstage-hex INSERT SQL transfer rows in Steps 4 and 5 | Not Calibrated - Not Benign | transport | TONESHELL implant receives `FILE_DOWNLOAD` (id=3) task and writes hex-encoded INSERT SQL file to `C:\Windows\Temp\` on WS01 | WS01 (10.12.10.30) | TESTLAB\labuser | [controlShell](../resources/payloads/rce-and-c2/mustang-panda-emulation/controlShell/) | - |
| sqlcmd.exe on WS01 bulk-INSERTs hex-encoded FwPolicySvc.exe chunks into tempdb..stg on IIS01 via MSSQL channel | Lateral Movement | T1570 | Lateral Tool Transfer | Windows | `sqlcmd.exe` spawned by `EssosUpdate.exe` on WS01 executes `-i C:\Windows\Temp\*.sql` against `10.12.10.20:1433` - a non-DBA, non-app-tier process bulk-inserting a SQL script into IIS01 MSSQL via the database channel has no established baseline on this developer workstation; same T1570 signal as the Step 4 xpstage-hex bulk-INSERT row | Not Calibrated - Not Benign | redundant@T1570 | TONESHELL spawns `sqlcmd -i <sql_file>` on WS01 to bulk-INSERT hex-encoded `FwPolicySvc.exe` chunks into `tempdb..stg` on IIS01 - database used as covert staging channel, no file written to IIS01 disk | WS01 (10.12.10.30) / IIS01 (10.12.10.20) | TESTLAB\labuser / sa | [controlShell](../resources/payloads/rce-and-c2/mustang-panda-emulation/controlShell/) | - |
| WS01 implant spawns sqlcmd T-SQL batch CONVERT(VARBINARY,'0x'+@hex,2) hex-decodes tempdb..stg rows to raw binary on IIS01 | Stealth | T1140 | Deobfuscate/Decode Files or Information | Windows | N/A - C3: hex decode via T-SQL `CONVERT(VARBINARY(MAX), '0x'+@hex, 2)` executes entirely in-process within `sqlservr.exe` on IIS01 - no process creation, file write, registry, or network artifact from the decode step itself; the resulting binary write to disk is covered by the T1559.001 ADODB.Stream row | Not Calibrated - Not Benign | out-of-surface | T-SQL batch on IIS01: `SELECT @hex=@hex+CAST(chunk AS VARCHAR(MAX)) FROM tempdb..stg` concatenates hex rows, then `CONVERT(VARBINARY(MAX),'0x'+@hex,1)` decodes hex string to raw binary — entirely in-process within `sqlservr.exe`; no PowerShell or cmd.exe spawn | IIS01 (10.12.10.20) | sa | [controlShell](../resources/payloads/rce-and-c2/mustang-panda-emulation/controlShell/) | - |
| sp_OA ADODB.Stream hex-decodes tempdb..stg rows and writes FwPolicySvc.exe binary to C:\ProgramData\ on IIS01 | Execution | T1559.001 | Inter-Process Communication: Component Object Model | Windows | `sqlservr.exe` creates `FwPolicySvc.exe` in `C:\ProgramData\` on IIS01 via COM automation (`ADODB.Stream`) - MSSQL service has no baseline for writing PE binaries to the system data directory via OLE Automation; no PowerShell or `cmd.exe` spawn on IIS01 (xpstage-hex is entirely in-process); Sysmon EC=11 on `FwPolicySvc.exe` with creator `sqlservr.exe` | Not Calibrated - Not Benign | redundant@T1559.001 | WS01 sqlcmd invokes T-SQL batch on IIS01: concatenates hex rows from `tempdb..stg` → `CONVERT(VARBINARY(MAX),@hex,2)` → `sp_OACreate 'ADODB.Stream'` + `sp_OASetProperty Type 1` + `sp_OAMethod Write` + `sp_OAMethod SaveToFile C:\ProgramData\FwPolicySvc.exe 2` — entire hex decode and binary write in-process within `sqlservr.exe`, no PowerShell or `cmd.exe` on IIS01 | IIS01 (10.12.10.20) | sa | [FwPolicySvc.exe](../resources/payloads/defense-impair/FwPolicySvc/) | - |
| hex INSERT SQL file deleted from C:\Windows\Temp\ on WS01 and tempdb..stg dropped on IIS01 | Stealth | T1070.004 | Indicator Removal: File Deletion | Windows | `EssosUpdate.exe` spawns `cmd.exe /c del /f` to delete the hex INSERT SQL file from `C:\Windows\Temp\` on WS01 - TONESHELL implant removing the staging artifact post-transfer; IIS01-side `DROP TABLE tempdb..stg` is N/A - C3 (internal MSSQL T-SQL operation, off EDR surface) | Not Calibrated - Not Benign | staging | WS01 deletes hex INSERT SQL file via `cmd /c del /f`; WS01 sqlcmd issues `DROP TABLE tempdb..stg` on IIS01 - staging artifacts cleared | WS01 (10.12.10.30) / IIS01 (10.12.10.20) | TESTLAB\labuser / sa | - | - |
| sp_OA WScript.Shell.Run dispatches CertEnrollSvc.exe SYSTEM chain running FwPolicySvc.exe via cmd.exe on IIS01 | Execution | T1559.001 | Inter-Process Communication: Component Object Model | Windows | `sqlservr.exe` on IIS01 spawns `CertEnrollSvc.exe` via OLE Automation (`sp_OACreate 'WScript.Shell'` + `sp_OAMethod Run`) - `sqlservr.exe` is the direct parent of `CertEnrollSvc.exe` with no intermediate `cmd.exe`; COM-based process creation from the MSSQL service has no baseline on this IIS01 instance; same COM dispatch signal as the Step 4 CertEnrollSvc.exe row | Not Calibrated - Not Benign | redundant@T1559.001 | `xprun` dispatches `CertEnrollSvc.exe` via sp_OA `WScript.Shell.Run` (ShellExecuteEx) — `sqlservr.exe` spawns `CertEnrollSvc.exe` with no intermediate `cmd.exe`; the tool performs the EfsPotato coercion and `CreateProcessWithTokenW` spawns `cmd /c C:\ProgramData\FwPolicySvc.exe setprofiles ...` as `NT AUTHORITY\SYSTEM`; `cmd.exe` is the stdout-redirection wrapper only | IIS01 (10.12.10.20) | NT SERVICE\MSSQL$SQLEXPRESS / NT AUTHORITY\SYSTEM | [CertEnrollSvc.exe](../resources/payloads/priv-escalation/EfsPotato/) | - |
...
| OPENROWSET BULK reads FwPolicySvc setprofiles output from C:\ProgramData\fwp_out.txt on IIS01 | Collection | T1005 | Data from Local System | Windows | `sqlservr.exe` on IIS01 opens `C:\ProgramData\fwp_out.txt` for read access via `OPENROWSET(BULK ... SINGLE_CLOB)` - EDR file-read telemetry: sqlservr.exe reading non-database `.txt` from `C:\ProgramData\` has no baseline on this IIS01 instance; native T-SQL operation, no COM automation or cmd.exe spawn | Not Calibrated - Not Benign | redundant@T1005 | `xpfile cat C:\ProgramData\fwp_out.txt` — `OPENROWSET(BULK ... SINGLE_CLOB)` reads the tool's stdout capture as `NVARCHAR(MAX)` in-process within `sqlservr.exe`; output returned as SELECT result to operator via C2 — no cmd.exe spawn on IIS01 | IIS01 (10.12.10.20) | sa | - | - |
