...
2. ☣️ On WS01 as `TESTLAB\labuser`: enqueue the BITS transfer of the delivery ZIP:
   ...
   > `BitsDownloader.exe` only creates and resumes the job; the transfer is performed by the BITS service (`svchost.exe -k netsvcs -s BITS`). The ZIP must be served by the standalone Range-capable `server.py` on TCP 8080 from `/media/sf_share` - see Setup.md → Steps 1B + 1C - shared `server.py` (`simplefileserver` on TCP 80 does not advertise `Accept-Ranges`).
...
Rather than put the payload on the wire as a download, the adversary takes a third delivery path. The lure document's hyperlink now points at an HTML page instead of the ZIP. The page impersonates a Microsoft Entra ID device-compliance check: it spins for a few seconds, then - entirely inside the browser - reconstructs an inert `.txt` file (a polyglot that looks like a PEM certificate bundle) from a base64 blob embedded in the page markup. No HTTP response ever carries that file, so proxy and DLP controls that inspect downloads by name or MIME type see only a routine page load. The page then presents a two-step remediation guide, pre-loads a one-line PowerShell command into the clipboard, and tells the user to press Win+R and paste it.

Running that command has `powershell.exe` read the `.txt` back, extract the base64 body, decode it to `%TEMP%\Essos_Compliance_Update.bin`, rename it to `Essos_Compliance_Update.hta`, and launch it with `mshta.exe`. Because the HTA is created by PowerShell and not by the browser, it carries no Mark-of-the-Web and mshta runs it in the Local Machine zone, where `ADODB.Stream` is not blocked. The HTA's embedded VBScript then writes the ToneShell sideload loader - `EssosUpdate.exe` and its hijacked `wsdapi.dll` - from base64 blobs carried inside the HTA into `%TEMP%`, and launches the loader with a hidden window. From that point the chain is identical to Step 1: the loader sideloads `wsdapi.dll` from its own directory, passes the sandbox checks, injects into `waitfor.exe`, and registers the TONESHELL C2 session. The ZIP and the LNK are bypassed entirely.

### Procedures

1. Operator: via RDP to WS01 as `TESTLAB\labuser`, copy `Essos_Compliance_Update.docx` to `C:\Users\labuser\Desktop\`

   > Variant lure document: same template as Step 1's `Braavos_Competitiveness_Brief.docx` but the embedded hyperlink points to `http://192.168.56.2:8080/staging.html` - served by the standalone `server.py` on TCP 8080 from `/media/sf_share` (see Setup.md → Steps 1B + 1C - shared `server.py`)

2. ☣️ On WS01 as `TESTLAB\labuser`: double-click `Desktop\Essos_Compliance_Update.docx` to open in Word, then CTRL+click the embedded link
   ...
   - ***Expected Output***
     ...
     Page reveals a two-step UI and pre-loads the Win+R command into the clipboard
     Essos_Compliance_Update.cer is saved to C:\Users\labuser\Downloads\
     No network request is made for the .txt file itself
     ```

3. ☣️ On WS01 as `TESTLAB\labuser`: press **Win+R**, paste the pre-loaded command (**Ctrl+V**), press **Enter**
   ...
   - ***Expected Output***
     ```text
     powershell.exe runs with -w h -ep bypass -c "iex(gc -Raw ...Essos_Compliance_Update.cer)"
     C:\Users\labuser\AppData\Local\Temp\Essos_Compliance_Update.bin appears, then is renamed to Essos_Compliance_Update.hta
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports |
| - | - | - | - | - | - | - | - | - | - | - | - | - |
| winword.exe browser navigation to staging.html lure page | Execution | T1204.001 | User Execution: Malicious Link | Windows | `winword.exe` spawns a browser child process (`msedge.exe` or `chrome.exe`) navigating to `http://192.168.56.2:8080/staging.html` - Office spawning a browser with a raw-IP HTTP URL is absent from the WS01 Word baseline | Not Calibrated - Not Benign | transport | `labuser` opens `Essos_Compliance_Update.docx` and CTRL+clicks the embedded hyperlink; `winword.exe` triggers browser navigation to `http://192.168.56.2:8080/staging.html` | WS01 (10.12.10.30) | TESTLAB\labuser | [staging.html](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| browser reconstructs Essos_Compliance_Update.cer from embedded base64 | Stealth | T1027.006 | Obfuscated Files or Information: HTML Smuggling | Windows | `msedge.exe`/`chrome.exe` writes `C:\Users\labuser\Downloads\Essos_Compliance_Update.cer` with no matching HTTP response carrying that file - client-side assembly from an embedded base64 blob; baseline: WS01 browser writes to Downloads are preceded by a matching network fetch | Calibrated - Not Benign | - | `staging.html` rebuilds `Essos_Compliance_Update.cer` from an embedded base64 blob and writes it to `Downloads\` with no corresponding HTTP GET for the file | WS01 (10.12.10.30) | TESTLAB\labuser | [staging.html](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| labuser Win+R paste of pre-loaded PowerShell launcher command | Execution | T1204.004 | User Execution: Malicious Copy and Paste | Windows | `explorer.exe` spawns `powershell.exe` directly from the Run dialog (no shell parent) with `-w h -ep bypass -c "iex(gc -Raw ...\Downloads\Essos_Compliance_Update.cer)"` - a hidden-window interpreter launched by the desktop shell is absent from the WS01 baseline | Not Calibrated - Not Benign | transport | Page pre-loads the launcher into the clipboard; `labuser` presses Win+R, pastes, and presses Enter | WS01 (10.12.10.30) | TESTLAB\labuser | [staging.html](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| powershell.exe -w h -ep bypass iex(gc -Raw) polyglot launcher | Execution | T1059.001 | Command and Scripting Interpreter: PowerShell | Windows | `powershell.exe` executes with `-w h -ep bypass -c "iex(gc -Raw '%USERPROFILE%\Downloads\Essos_Compliance_Update.cer')"` - hidden window + execution-policy bypass + `iex` of a Downloads text file; baseline: no such PowerShell invocation on WS01 | Not Calibrated - Not Benign | interpreter-spawn | `powershell.exe` runs with `-w h -ep bypass -c "iex(gc -Raw '%USERPROFILE%\Downloads\Essos_Compliance_Update.cer')"` | WS01 (10.12.10.30) | TESTLAB\labuser | [Essos_Compliance_Update.cer](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| powershell.exe FromBase64Transform decode of polyglot PEM body | Stealth | T1140 | Deobfuscate/Decode Files or Information | Windows | `powershell.exe` invokes `System.Security.Cryptography.FromBase64Transform` over the contents of `Downloads\Essos_Compliance_Update.cer` and writes the decoded bytes to `%TEMP%\Essos_Compliance_Update.bin` - base64-decode of a user-downloaded file; baseline: FromBase64Transform absent on WS01 | Calibrated - Not Benign | - | PowerShell reads the `.txt`, concatenates the base64 lines, and decodes them via `Security.Cryptography.FromBase64Transform` into a memory stream | WS01 (10.12.10.30) | TESTLAB\labuser | [Essos_Compliance_Update.cer](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| powershell.exe .bin to .hta rename of decoded payload | Stealth | T1036.008 | Masquerading: Masquerade File Type | Windows | `powershell.exe` creates `%TEMP%\Essos_Compliance_Update.hta` by renaming `%TEMP%\Essos_Compliance_Update.bin` - a decoded payload re-extensioned from `.bin` to executable `.hta`; baseline: no interpreter writes a `.hta` into `%TEMP%` on WS01 | Calibrated - Not Benign | - | PowerShell writes `%TEMP%\Essos_Compliance_Update.bin` then renames it to `%TEMP%\Essos_Compliance_Update.hta` | WS01 (10.12.10.30) | TESTLAB\labuser | [Essos_Compliance_Update.cer](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| powershell.exe mshta.exe HTA launch | Stealth | T1218.005 | System Binary Proxy Execution: Mshta | Windows | `powershell.exe` spawns `mshta.exe` with `%TEMP%\Essos_Compliance_Update.hta` as its argument - a scripting interpreter launching the signed `mshta.exe` proxy on a temp HTA; baseline: no powershell→mshta parent-child pair on WS01 | Calibrated - Not Benign | - | PowerShell spawns `mshta.exe` on `%TEMP%\Essos_Compliance_Update.hta` | WS01 (10.12.10.30) | TESTLAB\labuser | [Essos_Compliance_Update.cer](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| mshta.exe Window_OnLoad VBScript HTA execution | Execution | T1059.005 | Command and Scripting Interpreter: Visual Basic | Windows | `mshta.exe` executes inline VBScript from `%TEMP%\Essos_Compliance_Update.hta` instantiating `MSXML2.DOMDocument`, `ADODB.Stream`, and `Shell.Application` - AMSI/script telemetry records VBScript COM instantiation from a temp HTA; baseline: mshta running inline VBScript is absent on WS01 | Calibrated - Not Benign | - | HTA `Window_OnLoad` VBScript instantiates `MSXML2.DOMDocument`, `ADODB.Stream`, and `Shell.Application` COM objects to decode and stage the loader | WS01 (10.12.10.30) | TESTLAB\labuser | [hpsolutionsportal.hta](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| mshta.exe EssosUpdate.exe and wsdapi.dll embedded base64 drop | Stealth | T1027.009 | Obfuscated Files or Information: Embedded Payloads | Windows | `mshta.exe` writes two PE binaries - `EssosUpdate.exe` and `wsdapi.dll` - to `C:\Users\labuser\AppData\Local\Temp\`, decoded from base64 embedded in the HTA; baseline: mshta.exe creating executable images in `%TEMP%` is absent on WS01 | Calibrated - Not Benign | - | `mshta.exe` writes two PE binaries - `EssosUpdate.exe` and `wsdapi.dll` - to `%TEMP%`, decoded from base64 blobs embedded in the HTA | WS01 (10.12.10.30) | TESTLAB\labuser | [hpsolutionsportal.hta](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
| mshta.exe hidden EssosUpdate.exe loader launch | Stealth | T1564.003 | Hide Artifacts: Hidden Window | Windows | `mshta.exe` spawns `%TEMP%\EssosUpdate.exe` as a hidden child process (`Shell.Application.ShellExecute`, `nShow=0`) then exits - mshta launching a freshly-dropped `%TEMP%` PE with no visible window; baseline: mshta.exe has no child-process baseline on WS01 | Not Calibrated - Not Benign | C2 | `mshta.exe` calls `Shell.Application.ShellExecute` on `%TEMP%\EssosUpdate.exe` with `nShow=0` (hidden), then closes itself | WS01 (10.12.10.30) | TESTLAB\labuser | [hpsolutionsportal.hta](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
...
| TONESHELL EXEC cmd.exe /c del WNetHelper.exe on WS01 | Execution | T1059.003 | Command and Scripting Interpreter: Windows Command Shell | Windows | `waitfor.exe` (TONESHELL implant) spawns `cmd.exe /c del /f C:\Windows\Temp\WNetHelper.exe` on WS01 - process-creation telemetry (Sysmon EC=1 / Security 4688) records a `cmd.exe` whose command line is a `/c` batch carrying a `del` of a recently-written binary and whose parent image is `waitfor.exe`; baseline: Microsoft `Waitfor.exe` is a synchronisation helper invoked by admin/CI scripts on WS01 and never spawns a command shell, so both the unexpected shell-spawning parent and the shell child with a deletion command line are anomalous | Not Calibrated - Not Benign | interpreter-spawn | waitfor.exe (TONESHELL implant) spawns cmd.exe /c del /f C:\Windows\Temp\WNetHelper.exe - command-shell wrapper for tool cleanup on WS01 | WS01 (10.12.10.30) | TESTLAB\labuser | - | - |
