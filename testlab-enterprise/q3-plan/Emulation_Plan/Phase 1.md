...
Running that command has `powershell.exe` read the `.cer` back, extract the base64 body, decode it to `%TEMP%\Essos_Compliance_Update.bin`, rename it to `hpsolutionsportal.hta`, and launch it with `mshta.exe`. Because the HTA is created by PowerShell and not by the browser, it carries no Mark-of-the-Web and mshta runs it in the Local Machine zone, where `ADODB.Stream` is not blocked. The HTA's embedded VBScript then writes the ToneShell sideload loader - `EssosUpdate.exe` and its hijacked `wsdapi.dll` - from base64 blobs carried inside the HTA into `%TEMP%`, and launches the loader with a hidden window. From that point the chain is identical to Step 1: the loader sideloads `wsdapi.dll` from its own directory, passes the sandbox checks, injects into `waitfor.exe`, and registers the TONESHELL C2 session. The ZIP and the LNK are bypassed entirely.
...
3. ☣️ On WS01 as `TESTLAB\labuser`: press **Win+R**, paste the pre-loaded command (**Ctrl+V**), press **Enter**
   ...
   - ***Expected Output***
     ...
     C:\Users\labuser\AppData\Local\Temp\Essos_Compliance_Update.bin appears, then is renamed to hpsolutionsportal.hta
