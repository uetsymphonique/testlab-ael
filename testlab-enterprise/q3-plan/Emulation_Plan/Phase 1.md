...
2. ☣️ On WS01 as `TESTLAB\labuser`: double-click `Desktop\Essos_Compliance_Update.docx` to open in Word, then CTRL+click the embedded link
   ...
   - ***Expected Output***
     ...
     Essos_Compliance_Update.txt is saved to C:\Users\labuser\Downloads\
...
3. ☣️ On WS01 as `TESTLAB\labuser`: press **Win+R**, paste the pre-loaded command (**Ctrl+V**), press **Enter**
   ...
   - ***Expected Output***
     ```text
     powershell.exe runs with -w h -ep bypass -c "iex(gc -Raw ...Essos_Compliance_Update.txt)"
...
| mshta.exe EssosUpdate.exe and wsdapi.dll embedded base64 drop | Stealth | T1027.009 | Obfuscated Files or Information: Embedded Payloads | Windows | `mshta.exe` writes two PE binaries - `EssosUpdate.exe` and `wsdapi.dll` - to `C:\Users\labuser\AppData\Local\Temp\`, decoded from base64 embedded in the HTA; baseline: mshta.exe creating executable images in `%TEMP%` is absent on WS01 | Calibrated - Not Benign | - | `mshta.exe` writes two PE binaries - `EssosUpdate.exe` and `wsdapi.dll` - to `%TEMP%`, decoded from base64 blobs embedded in the HTA | WS01 (10.12.10.30) | TESTLAB\labuser | [stage1.hta](../resources/payloads/user-trigger/html-smuggling-hta/) | - |
