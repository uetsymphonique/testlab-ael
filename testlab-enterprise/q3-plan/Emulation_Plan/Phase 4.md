...
<!-- Every xprun-out invocation of go-thehash.exe in this phase runs via sp_OA WScript.Shell.Run (ShellExecuteEx) as NT SERVICE\MSSQL$SQLEXPRESS - the same COM-dispatch primitive scored in Phase 2 Step 4B ("sp_OA WScript.Shell.Run dispatches CertEnrollSvc.exe", T1559.001) - and is not re-scored here. No EfsPotato token theft or CreateProcessWithTokenW is involved on IIS01 in this phase. -->
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports |
| - | - | - | - | - | - | - | - | - | - | - | - | - |
| PolicySyncSvc.exe XOR-decodes string table and resolves kernel32 exports via GetProcAddress | Stealth | T1027.007 | Obfuscated Files or Information: Dynamic API Resolution | Windows | `PolicySyncSvc.exe` on DC01 carries a sparse import table (only `GetModuleHandleW` and `GetProcAddress`) and resolves `CreateFileW` / `DeviceIoControl` / `ReadFile` at runtime via `GetProcAddress` after decoding a position-keyed XOR string table - YARA / capability-signature match on the file flags the two-import IAT together with a runtime XOR-decode routine, and native API monitoring observes `GetProcAddress`-based resolution of raw-volume APIs at load; baseline: no legitimately-installed binary on DC01 has a two-import IAT - the static artifact is evaluator-verifiable from the file on disk without trusting the implant | Calibrated - Not Benign | - | PolicySyncSvc.exe decodes its position-keyed XOR string table (WMI class/namespace and file paths) at runtime and resolves CreateFileW/DeviceIoControl/GetFileSizeEx/ReadFile from kernel32 via GetProcAddress - sparse import table (only GetModuleHandleW/GetProcAddress) hides its WMI and raw-volume-read capability from static analysis | DC01 (TBD) | TESTLAB\Administrator | [NtdsRawDump](../resources/payloads/cred-access/NtdsRawDump/) | - |
