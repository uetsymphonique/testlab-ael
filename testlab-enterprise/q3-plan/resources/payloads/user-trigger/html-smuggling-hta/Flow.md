...
| 11 | powershell renames %TEMP%\Essos_Compliance_Update.bin to hpsolutionsportal.hta (removing any pre-existing .hta) | %TEMP%\hpsolutionsportal.hta [file] → #12 | — | Remove-Item -Force then Rename-Item; powershell-created file carries no Zone.Identifier ADS |
| 12 | powershell launches mshta.exe on %TEMP%\hpsolutionsportal.hta | mshta.exe process [process] → #13 | — | HTA runs in Local Machine zone (no MotW); WINDOWSTATE=minimize, SHOWINTASKBAR=no |
