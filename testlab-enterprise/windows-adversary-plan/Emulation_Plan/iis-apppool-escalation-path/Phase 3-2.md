...
| - | - | - | - | - | - | - | - | - | - | - | - | - | - |
| go-thehash.exe PtH NTLM network logon to DC01 from IIS01 | Lateral Movement | T1550.002 | Use Alternate Authentication Material: Pass the Hash | Windows | `go-thehash.exe` on IIS01 initiates outbound SMB connection to DC01 (10.12.10.10:445) and NTLM-authenticates as TESTLAB\Administrator | Calibrated - Not Benign | - | `go-thehash.exe put` authenticates to DC01 using the Administrator NT hash, bypassing the password requirement to open the C$ admin share | IIS01/react.testlab.local → DC01 | TESTLAB\Administrator | [main.go connect()](../../resources/payloads/lateral-movement/go-thehash/main.go) | -|
| go-thehash.exe SMB2 admin share access to DC01 C$ | Lateral Movement | T1021.002 | Remote Services: SMB/Windows Admin Shares | Windows | SMB client on IIS01 accesses `\\DC01\C$` admin share as TESTLAB\Administrator | Not Calibrated - Not Benign | remote-exec | `go-thehash.exe put` opens an SMB2 tree to `\\DC01\C$`, using the PtH session to access the admin share for payload staging | IIS01/react.testlab.local → DC01 | TESTLAB\Administrator | [main.go putFile()](../../resources/payloads/lateral-movement/go-thehash/main.go) | -|
| go-thehash.exe SMB C$ write of persistence binaries to DC01 C:\ProgramData | Lateral Movement | T1570 | Lateral Tool Transfer | Windows | unsigned PE executables policyupdate.exe, policysync.exe, policysync-host.exe, ServiceInstaller.exe, and NtServiceInstaller.exe written to C:\ProgramData on DC01 via SMB from IIS01 (10.12.10.20) | Not Calibrated - Not Benign | transport | `go-thehash.exe put` writes four persistence binaries into DC01's `C:\ProgramData\` over the C$ admin share | IIS01/react.testlab.local → DC01 | TESTLAB\Administrator | [main.go putFile()](../../resources/payloads/lateral-movement/go-thehash/main.go) | -|
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports
| - | - | - | - | - | - | - | - | - | - | - | - | - | - |
| net.exe domain account creation of svcbackup on DC01 | Persistence | T1136.002 | Create Account: Domain Account | Windows | `net.exe` on DC01 creates a domain user account — command line contains `user`, `/add`, and `/domain` | Calibrated - Not Benign | - | `net.exe user svcbackup /add /domain` creates a backdoor domain account on DC01 | DC01 | TESTLAB\Administrator | - | -|
| net.exe Domain Admins group membership addition of svcbackup on DC01 | Persistence | T1098.007 | Account Manipulation: Additional Local or Domain Groups | Windows | `net.exe` on DC01 adds a user to the Domain Admins group — command line contains `group`, `"Domain Admins"`, and `/add` | Calibrated - Not Benign | - | `net.exe group "Domain Admins" svcbackup /add /domain` escalates the backdoor account to Domain Admin on DC01 | DC01 | TESTLAB\Administrator | - | -|
| svcbackup backdoor domain account credential-based re-entry on DC01 | Persistence | T1078.002 | Valid Accounts: Domain Accounts | Windows | a domain account created by `net.exe` with Domain Admins membership exists on DC01 — enabling credential-based re-entry via SMB, WinRM, or RDP | Not Calibrated - Not Benign | redundant@T1136.002 | Backdoor domain account `svcbackup` with Domain Admin membership enables credential-based re-entry via SMB, WinRM, or RDP from any network-connected host | DC01 | TESTLAB\Administrator (creator) | - | -|
| reg.exe SpecialAccounts\UserList registry write to hide svcbackup from logon screen on DC01 | Defense Evasion | T1564.002 | Hide Artifacts: Hidden Users | Windows | `reg.exe` on DC01 writes a value under `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon\SpecialAccounts\UserList` | Calibrated - Not Benign | - | `reg.exe` writes `svcbackup=0` under `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon\SpecialAccounts\UserList` on DC01, hiding the backdoor account from the logon screen | DC01 | TESTLAB\Administrator | - | -|
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports
| - | - | - | - | - | - | - | - | - | - | - | - | - | - |
| powershell.exe Set-WmiInstance WMI subscription object creation in root\subscription on DC01 | Execution | T1059.001 | Command and Scripting Interpreter: PowerShell | Windows | `powershell.exe` on DC01 creates WMI permanent event subscription objects — command line contains `Set-WmiInstance` and `__FilterToConsumerBinding` | Not Calibrated - Not Benign | interpreter-spawn | `powershell.exe` invokes `Set-WmiInstance` to create timer-triggered WMI subscription objects in `root\subscription` on DC01, with `CertPolicyConsumer` pointing to `C:\ProgramData\policyupdate.exe` | DC01 | TESTLAB\Administrator | - | -|
| wmiprvse.exe WMI permanent event subscription CertPolicyTimer firing of policyupdate.exe on DC01 | Persistence | T1546.003 | Event Triggered Execution: Windows Management Instrumentation Event Subscription | Windows | `wmiprvse.exe` on DC01 spawns an unsigned PE executable from `C:\ProgramData` — WMI permanent event subscription execution; Event ID 5861 (Microsoft-Windows-WMI-Activity/Operational) records the subscription binding | Calibrated - Not Benign | - | WMI permanent event subscription created: `CertPolicyConsumer` executes `C:\ProgramData\policyupdate.exe` when the 60-second `CertPolicyTimer` fires; `wmiprvse.exe` spawns `policyupdate.exe` directly as SYSTEM | DC01 | TESTLAB\Administrator (creator) / NT AUTHORITY\SYSTEM (runtime) | [policyupdate.exe](../../resources/payloads/rce-and-c2/dnscat2/go-client/dnscat2.exe) | -|
| CertPolicyFilter and CertPolicyConsumer WMI object name masquerading as certificate policy on DC01 | Defense Evasion | T1036.005 | Masquerading: Match Legitimate Resource Name or Location | Windows | WMI subscription objects in `root\subscription` on DC01 use names that follow a legitimate-looking naming pattern — paired with a `CommandLineEventConsumer` that executes an unsigned PE from `C:\ProgramData` | Calibrated - Not Benign | - | WMI subscription objects named `CertPolicyFilter` and `CertPolicyConsumer` to blend with Windows certificate-policy infrastructure | DC01 | TESTLAB\Administrator | - | -|
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Calibration Reason | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports
| - | - | - | - | - | - | - | - | - | - | - | - | - | - |
| cmd.exe copy of update.exe to SYSVOL scripts folder on DC01 | Lateral Movement | T1570 | Lateral Tool Transfer | Windows | `cmd.exe` on DC01 copies an unsigned PE executable to `C:\Windows\SYSVOL\sysvol\testlab.local\scripts\` — SYSVOL scripts directory modified outside Group Policy processing | Calibrated - Not Benign | - | `cmd.exe copy` places `policyupdate.exe` into the SYSVOL scripts folder as `update.exe` on DC01; DFS-R replication makes it available as `\\testlab.local\SYSVOL\testlab.local\scripts\update.exe` from all domain controllers | DC01 | TESTLAB\Administrator | [policyupdate.exe](../../resources/payloads/rce-and-c2/dnscat2/go-client/dnscat2.exe) | -|
| powershell.exe scripts.ini GPO modification in Default Domain Policy on DC01 | Defense Evasion | T1484.001 | Domain or Tenant Policy Modification: Group Policy Modification | Windows | `powershell.exe` on DC01 writes `scripts.ini` under the Default Domain Policy SYSVOL path — registering a user logon script pointing to an unsigned PE via UNC path | Calibrated - Not Benign | - | `powershell.exe` writes `scripts.ini` into the Default Domain Policy SYSVOL path on DC01, registering `\\testlab.local\SYSVOL\testlab.local\scripts\update.exe` as the domain logon script | DC01 | TESTLAB\Administrator | - | -|
| userinit.exe logon script execution of update.exe from SYSVOL on WS01 | Persistence | T1037.003 | Boot or Logon Initialization Scripts: Network Logon Script | Windows | `userinit.exe` on WS01 spawns `update.exe` from `\\testlab.local\SYSVOL\testlab.local\scripts\` — an unsigned PE executable loaded from SYSVOL as a domain logon script | Calibrated - Not Benign | - | `userinit.exe` reads `scripts.ini` and spawns `update.exe` (dnscat2) in the logged-on domain user's session on WS01 on next domain logon | WS01 | TESTLAB\<domain user> | [dnscat2.exe](../../resources/payloads/rce-and-c2/dnscat2/go-client/dnscat2.exe) | -|
...
| - | - | - | - | - | - | - | - | - | - | - | - | - |
| NtServiceInstaller.exe NT native API registry-backed service creation of CertPolicyCache on DC01 | Persistence | T1543.003 | Create or Modify System Process: Windows Service | Windows | NtServiceInstaller.exe creates registry key HKLM\SYSTEM\CurrentControlSet\Services\CertPolicyCache with Start=2 and ImagePath=C:\ProgramData\policysync.exe | Calibrated - Not Benign | - | `NtServiceInstaller.exe install` creates an auto-start service by writing the service configuration directly to the Services registry hive; the persistent service definition is independently verifiable even when System Event ID 7045 is absent | DC01 | TESTLAB\Administrator | [NtServiceInstaller.exe](../../resources/payloads/persistence/windows-service/syscalls-cpp/NtServiceInstaller.exe), [service_installer.cpp](../../resources/payloads/persistence/windows-service/syscalls-cpp/service_installer.cpp) | -|
| CertPolicyCache registry-backed service name masquerading as certificate policy on DC01 | Defense Evasion | T1036.004 | Masquerading: Masquerade Task or Service | Windows | CertPolicyCache service registry key has display name "Certificate Policy Cache" and description "Caches certificate policy metadata" mimicking Windows certificate infrastructure, but ImagePath points to an unsigned PE in C:\ProgramData | Not Calibrated - Not Benign | redundant@T1543.003 | The service name, display name, and description are chosen to disguise the malicious service as a benign certificate-policy component; this is an evasion attribute of the same registry-backed service object already scored under `T1543.003` | DC01 | TESTLAB\Administrator | [service_installer.cpp InstallService()](../../resources/payloads/persistence/windows-service/syscalls-cpp/service_installer.cpp) | -|
| NtServiceInstaller.exe GetProcAddress ntdll.dll dynamic resolution for native registry API on DC01 | Execution | T1106 | Native API | Windows | NtServiceInstaller.exe invokes NtCreateKey and NtSetValueKey syscalls to write service configuration under HKLM\SYSTEM\CurrentControlSet\Services\CertPolicyCache, without importing advapi32.dll registry functions | Calibrated - Not Benign | - | `NtServiceInstaller.exe install` uses dynamic ntdll.dll resolution (`GetProcAddress`) to invoke NT native registry APIs, intentionally avoiding advapi32.dll hooks; this requires API-call–level monitoring (ETW kernel callbacks) to detect, a different telemetry depth than the Sysmon 12/13 registry-object events that capture T1543.003 | DC01 | TESTLAB\Administrator | [nt_api.cpp](../../resources/payloads/persistence/windows-service/syscalls-cpp/nt_api.cpp), [service_installer.cpp](../../resources/payloads/persistence/windows-service/syscalls-cpp/service_installer.cpp) | -|
| NtServiceInstaller.exe NT native API registry value modification of CertPolicyCache service key on DC01 | Defense Evasion | T1112 | Modify Registry | Windows | NtServiceInstaller.exe writes registry values ImagePath, Start, and ObjectName under HKLM\SYSTEM\CurrentControlSet\Services\CertPolicyCache using NT native registry syscalls | Not Calibrated - Not Benign | redundant@T1543.003 | Registry modification is the mechanism used to create the Windows service; this would double-count the same persistent service artifact already represented by T1543.003 | DC01 | TESTLAB\Administrator | [service_installer.cpp InstallService()](../../resources/payloads/persistence/windows-service/syscalls-cpp/service_installer.cpp) | -|
...
| Summary | Tactic | Technique ID | Technique Name | Platform | Detection Criteria | Category | Red Team Activity | Hosts | Users | Source Code Links | Relevant CTI Reports
| - | - | - | - | - | - | - | - | - | - | - | - | - |
| ServiceInstaller.exe CreateServiceW SCM API service creation of CertPolicyHost on DC01 | Persistence | T1543.003 | Create or Modify System Process: Windows Service | Windows | ServiceInstaller.exe creates Windows service CertPolicyHost via CreateServiceW with Start=2 and ImagePath=C:\ProgramData\policysync-host.exe, generating System Event ID 7045 | Calibrated - Not Benign | - | `ServiceInstaller.exe install` calls SCM APIs directly to create an auto-start service for `policysync-host.exe`; the persistent service object is independently verifiable through Event ID 7045 and the service registry key | DC01 | TESTLAB\Administrator | [ServiceInstaller.exe](../../resources/payloads/persistence/windows-service/advapi32-cpp/ServiceInstaller.exe), [service_installer.cpp](../../resources/payloads/persistence/windows-service/advapi32-cpp/service_installer.cpp) | -|
| CertPolicyHost SCM API service name masquerading as certificate policy on DC01 | Defense Evasion | T1036.004 | Masquerading: Masquerade Task or Service | Windows | CertPolicyHost service has display name "Certificate Policy Host" and description "Maintains certificate policy synchronization" mimicking Windows certificate infrastructure, but ImagePath points to an unsigned PE in C:\ProgramData | Not Calibrated - Not Benign | redundant@T1543.003 | The service name, display name, and description are chosen to blend with legitimate Windows certificate-policy infrastructure; this is an evasion attribute of the same service object already scored under `T1543.003`, not a separate persistence outcome | DC01 | TESTLAB\Administrator | [service_installer.cpp InstallService()](../../resources/payloads/persistence/windows-service/advapi32-cpp/service_installer.cpp) | -|
| services.exe SCM launch of policysync-host.exe as CertPolicyHost service on DC01 | Execution | T1569.002 | System Services: Service Execution | Windows | services.exe spawns policysync-host.exe from C:\ProgramData to run as the CertPolicyHost service | Calibrated - Not Benign | - | Start substep: `ServiceInstaller.exe start CertPolicyHost` triggers SCM to launch the already-created persistence service; execution confirms the service works but the scored behavior is the persistent service creation in T1543.003 | DC01 | NT AUTHORITY\SYSTEM | [service_installer.cpp StartServiceByName()](../../resources/payloads/persistence/windows-service/advapi32-cpp/service_installer.cpp) | -|
