...
| Collection | T1119 | Automated Collection | Windows | N/A — C1: automated chain leaves no observable signal independent of the individual technique rows in this table; no per-row anomaly axis exists at the T1119 level | Not Calibrated - Not Benign | C1 | `PolicySyncSvc.exe` automates VSS creation via WMI, raw cluster reads for four credential targets, per-file AES-256-CBC encryption, shadow deletion, in-memory ZipArchive, and outer AES-256-CBC encryption without operator intervention | DC01 (10.12.10.10) | TESTLAB\Administrator | [NtdsRawDump.cs](../../resources/payloads/cred-access/NtdsRawDump/NtdsRawDump.cs) | -
