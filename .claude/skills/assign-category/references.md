...
- **Downstream:** the downstream Calibrated row *directly proves* this one fired. Do not apply merely because a step is upstream — if it offers an earlier independent cut-point, it is not an implementation detail.
...
| T1105 write of a file with an independently observable anomaly (PE class, YARA-matchable content) → downstream row scores a different detection angle | No | The file itself carries a rebuttal-triggering property; the write event and the downstream detection require different capabilities |
...
**4b — New & independent detection opportunity.** A row earns a point only if it opens a detection chance another scored row does not already guarantee. Exfil over an *existing* C2 channel = NC (channel already scored); exfil over a *newly-opened* channel = Calibrated (separate cut-point). Ask: *"If every row this one depends on were already caught, would this still be its own detection opportunity?"*

**4c — Distinctive TTP vs. connective tissue.** Score actor-signature behaviors a competent product of the declared tier is genuinely expected to flag. Exclude generic connective tissue — missing it is not fairly attributable to a detection-capability gap:
...
> **Rebuttable priors, not technique-ID blocklists.** The same technique flips by its *role in the step* — `rar` staging is NC; `rar` + alternate-protocol exfil as the distinctive objective is Calibrated. Record *why* it is NC in this step.

> **Rebuttal trigger — when to probe instead of applying the prior mechanically.** Read the written criteria before applying any strong-prior NC. If the criteria carries a condition **beyond the prior's class definition** (a structural property of the artifact, a content fingerprint, a behavioral pattern intrinsic to the action — not merely actor + action + target), the prior is no longer the right description and the rebuttal must be tested explicitly. Tool transfer (T1105) writing a generic file = prior holds; tool transfer writing a binary with PE-format YARA detection = lifted out of the prior, test C4 fresh. Apply this probe to every prior in the C4c table, not just T1105.

> **Identifying the primary artifact before testing the rebuttal.** The rebuttal must be triggered by the **primary artifact** of the action — not by metadata about who performed it. For T1105, the primary artifact is the transferred file, not the writer process. An anomalous actor (IIS worker, browser, Office app writing to a suspicious path) is context; it does not by itself lift the transport prior. Ask: *does the file have an independently observable anomaly?* If yes, test C4 fresh. If no, check Q-B: a downstream Calibrated row that captures the file's distinctive property (e.g., a file-scan row for the same artifact) directly proves staging occurred → write event is `redundant@<that row>`. The same logic applies to other connective-tissue priors: always identify what the action *produces* as its primary artifact, then test whether *that artifact* carries the rebuttal-triggering property.
...
7. **C4 gate** — all three must hold:
   ...
   - **4c** Distinctive actor-signature TTP, not generic connective tissue? (No → NC)
...
| `transport` | Tool transfer / generic delivery (C4c — connective tissue) |
...
> `Category` stays a clean enum (filterable by tooling) — never put free text there. For a C1/C2 failure the detail already lives in `Detection Criteria` as `N/A — <code>: <reason>`; just mirror the short tag here. For NC on scope/redundancy/connective-tissue where `Detection Criteria` holds a *positive* signal, the positive signal stays as evidence; the short tag goes in `Calibration Reason`.
