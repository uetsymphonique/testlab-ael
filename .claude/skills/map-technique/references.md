...
These tactic files are large (Defense Evasion ≈ 3,300 lines, Persistence ≈ 1,800), because every technique carries a full `**Detection**` block and a long `**Procedure Examples**` list. **None of that is needed for mapping.** `Read`ing the whole file is what bloats context.
