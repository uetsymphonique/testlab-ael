...
4. For each behavior, record the **produces→consumes edge** (the artifact it leaves and which later behavior `#` reads it) and a neutral **baseline context** note where one is obvious (the anomaly-axis seed for `/write-detection-criteria`). Do **not** write the anomaly verdict, Category, or Detection Criteria.
...
7. **Suggest the next step:** once the user is satisfied, run `/map-technique` to fill the `Tactic / TID` column.
...
- **Tactic / TID — Technique Name** — leave as `—`; filled by the `/map-technique` pass. Format: `<Tactic> / <TID> — <Technique Name>`. Sub-technique preferred over parent when one fits.
- **Context (baseline)** — neutral one-liner; leave blank if none is obvious. Never an anomaly verdict.

## Notes

- The output of this skill is the behavior skeleton with `Tactic / TID = —`. The column is filled only after the user verifies and `/map-technique` runs.
- `Flow.md` is **per payload** (internal mechanics of one binary) — distinct from the Phase Reference Table (the operational, scored surface). The Phase table may collapse a whole payload into a few rows; `Flow.md` explains what is inside.
