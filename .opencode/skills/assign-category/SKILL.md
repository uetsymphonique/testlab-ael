...
description: Assign Calibrated / Not Calibrated labels to Reference Table rows in a Phase file - deciding which adversary behaviors count as scored detection opportunities
effort: high
...
**Objective:** calibrate the scoring surface of a Phase - decide which rows are *fair, scored detection opportunities* versus setup steps, implementation details, or artifacts outside the measurement surface (not scored). A row is Calibrated only when its artifact is **observable, reproducible, independently verifiable, and fairly scoreable** on the declared telemetry surface, and is not already represented better by another row. The output is an accurate `Category` column.
...
2. **Establish Surface Profile** (references.md → Surface Profile):
   ...
   - **Echo the active Surface Profile explicitly - never label on a silent default.** State which profile row is in force. This choice sets which rows are eligible to be scored detection opportunities.
...
**"The actor is anomalous, so the action is Calibrated."** Actor anomaly is context, not an override criterion. For Generic Operational Behavior default verdicts, the override must be triggered by the **primary artifact** the action produces - not by who performed it. For T1105, ask whether the transferred file has an independently observable anomaly. If not, the default verdict holds; check Q-B against any downstream row that captures the file's distinctive property.
